            ...
            <div class="w-[256px] flex flex-col relative cursor-pointer shadow-md">
                ...
                <div class="pt-2 pb-4 px-4 flex flex-col gap-2.5 justify-center bg-white rounded-bl-lg rounded-br-lg">
                    ...
                    <div class="flex items-center justify-between gap-2">
                        @if($product->discount_sale > 0)
                            <p
                                class="text-[#FF3750] text-sm md:text-lg font-semibold leading-[22px]">{{number_format($product->price - ($product->price * $product->discount_sale / 100),0,'.','.').'đ'}}</p>
                            <p class="text-[#696984] text-xs md:text-sm leading-[38px] line-through truncate">
                                {{number_format($product->price,0,'.','.').'đ'}}</p>
                        @else
                            <p
                                class="text-[#FF3750] text-sm md:text-lg font-semibold leading-[22px]">{{number_format($product->price,0,'.','.').'đ'}}
                            </p>
                        @endif
