        ...
        <div class="content  max-h-[600px] overflow-y-auto">
            <div class="flex flex-col justify-start items-start gap-4 py-3 w-full">
                <div class="grid grid-cols-2 gap-4 w-full">
                    <span class="text-title font-medium ">Họ tên:</span>
                    <span class="text-title">{{$user->name}}</span>
                </div>
                <div class="grid grid-cols-2 gap-4 w-full">
                    <span class="text-title font-medium ">Email:</span>
                    <span class="text-title">{{$user->email}}</span>
                </div>
                <div class="grid grid-cols-2 gap-4 w-full">
                    <span class="text-title font-medium ">ID người đại diện:</span>
                    <span class="text-title">{{$user->id_vdone ?? ''}}</span>
                </div>
                <div class="grid grid-cols-2 gap-4 w-full">
                    <span class="text-title font-medium ">Số điện thoại:</span>
                    <span class="text-title">{{$user->phone_number}}</span>
                </div>
                <div class="grid grid-cols-2 gap-4 w-full">
                    <span class="text-title font-medium ">Mã số thuế:</span>
                    <span class="text-title">{{$user->tax_code}}</span>
                </div>
                <div class="grid grid-cols-2 gap-4 w-full">
                    <span class="text-title font-medium ">Địa chỉ:</span>
                    <span class="text-title">{{$user->address}}</span>
                </div>
                <div class="grid grid-cols-2 gap-4 w-full">
                    <span class="text-title font-medium ">Ngày đăng ký:</span>
                    <span
                        class="text-title">{{\Illuminate\Support\Carbon::parse($user->created_at)->format('d/m/Y H:i')}}</span>
                </div>
            </div>
        </div>
    </div>
