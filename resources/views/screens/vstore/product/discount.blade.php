...
@section('content')
    <form action="" id="form">
        ...
        <a href="{{route('screens.vstore.product.discount')}}" class="text-blueMain font-medium italic">Quản lý giảm giá</a>
        </div>
...
@section('custom_js')
    <script>
        ...
        $('.btnAddDis').on('click', (o) => {
            $.ajax({
                url: '{{route('screens.vstore.product.createDis')}}?_token={{csrf_token()}}',
                success: function (result) {
                    $('#modal2').html(result);
                    $('.modal-details').toggleClass('show-modal')
                },
