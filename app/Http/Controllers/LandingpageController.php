...
//        return 1;
        ...
        if ($user) {
            ...
            $name = $user->name ?? '';
            $category = $user->products()->select("category_id")->where('availability_status', 1)->groupBy('category_id')->get();
...
//            return $category;
        ...
        $selected = [DB::raw('SUM(discounts.discount) as discount'), 'vote', 'images', 'publish_id', 'price', 'products.name', DB::raw("price - (price * IFNULL((SELECT SUM(discount /100) as dis
        ...
        $hot_products = DB::table('products')
            ->where('availability_status', 1);
        $selected = ['vote', 'images', 'publish_id', 'price', 'products.name'];
    ...
    public function vstore($slug)
    {
        ...
        if ($user) {
            ...
            $banner = !empty($user->banner) ? $user->banner : '';
            $name = $user->name ?? '';
            $category = $user->vstoreProducts()->select("category_id")->where('availability_status', 1)->groupBy('category_id')->get();
//            $category= Category::all();
//            return $category;
            $arrCategory = [];
            foreach ($category as $cate) {
