...
//            return $request ->option;
            ...
            $products = $products->select($selected)
                ->selectSub('SELECT price *
       IFNULL((SELECT SUM(discount) FROM discounts WHERE start_date <= NOW()
                                    and end_date >= NOW() AND product_id =
                                    products.id GROUP BY product_id),1) FROM products where id = products.id', 'order_price');
...
//            ]);
    ...
    function getProductAvailableByVshop(Request $request, $pdone_id)
    {
        ...
        foreach ($products as $pr) {
            $user_discount = DB::table('discounts')
                ->selectRaw('SUM(discount) as dis')
                ->where('product_id', $pr->id)
                ->where('start_date', '<=', Carbon::now())
                ->where('end_date', '>=', Carbon::now())
                ...
                ->first()->dis ?? 0;
            $vshop_discount = DB::table('discounts')
                ->selectRaw('SUM(discount) as dis')
                ->where('product_id', $pr->id)
                ->where('start_date', '<=', Carbon::now())
                ->where('end_date', '>=', Carbon::now())
