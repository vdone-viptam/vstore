...
//        return  $this->v['users'];
    ...
    public function vshop(Request $request)
    {
        ...
        $vshop = Vshop::join('vshop_products', 'vshop.id', '=', 'vshop_products.vshop_id')
            ...
            ->selectSub('SELECT SUM(`order_item`.price * `order_item`.quantity  ) FROM `order` JOIN order_item on `order`.id = order_item.order_id  WHERE export_status = 4 AND order_item.vshop_id = vshop.id AND products.vstore_id=' . Auth::id(), 'doanh_thu')
