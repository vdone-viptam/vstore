...
//        return  $this->v['users'];
    ...
    public function vshop(Request $request)
    {
        ...
        $vshop = Vshop::join('vshop_products', 'vshop.id', '=', 'vshop_products.vshop_id')
            ...
            ->selectSub('SELECT COUNT(vshop_products.product_id) FROM vshop_products JOIN products ON vshop_products.product_id = products.id WHERE vshop_products.vshop_id = vshop.id AND products.vstore_id =' . Auth::id().' AND vshop_products.status != 3 group by vshop_products.product_id', 'amount_product')
